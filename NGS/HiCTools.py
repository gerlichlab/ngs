...
def getExpected(
    clr: cooler.Cooler, arms: pd.DataFrame, proc: int = 20, ignoreDiagonals: int = 2
) -> pd.DataFrame:
...
def assignRegions2d(
    ...
    windowsSmall = windowsFinal[
        ["chrom1", "start1", "end1", "chrom2", "start2", "end2", "region1"]
    ]
    windowsSmall.columns = [
        "chrom1",
        "start1",
        "end1",
        "chrom2",
...
def slidingDiamond(
    array: np.ndarray, sideLen: int = 6, centerX: bool = True
    ...
    if sideLen % 2 == 0:
        ...
        for i in range(halfWindow, (array.shape[0] - halfWindow)):
            ...
            diamondAccumulator.append(np.nanmean(diamondArray))
            # append x-value for this particular bin
            binAccumulator.append(
                np.median(range(i - halfWindow, (i + halfWindow) + 1,))
            )
    if centerX:
...
def downSamplePairs(sampleDict: pairsSamples, Distance: int = 10 ** 4) -> pairsSamples:
    ...
    for sample in sampleDict.keys():
        ...
        # filter on distance
        outDict[sample]["all"] = outDict[sample]["all"].loc[
            (outDict[sample]["all"]["pos2"] - outDict[sample]["all"]["pos1"])
            > Distance,
            :,
        ]
    ...
    for sample in outDict.keys():
        ...
        outDict[sample]["cis"] = outDict[sample]["all"].loc[
            outDict[sample]["all"]["rType"] == "cis", :
        ]
        outDict[sample]["trans"] = outDict[sample]["all"].loc[
            outDict[sample]["all"]["rType"] == "trans", :
        ]
...
def getPairingScore(
    ...
    if blankDiag:
        dummyarray = np.arange(pile[:, :, 0].shape[0] ** 2).reshape(
...
def getPairingScoreObsExp(
    ...
    windows = windows.dropna()
    # generate pileup
    pile = doPileupObsExp(clr, expected, windows, collapse=False)
    # convert to dataframe
    pileFrame = pileToFrame(pile)
